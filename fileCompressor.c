...
void insert(Node** head, char* name, char* token){
...
char* compareStringtoLinkedList(Node* head, char* token){
    Node* n = head;
    while(n != NULL){
        if(string_equal(token, n->token)){
            return n->data;
        }
        n = n->next;
    }
...
int do_decompress(char* name){
    ...
    // }
    Node* head = NULL;
    int fileD = open("./HuffmanCodebook", O_RDONLY, 0744); // codebook
    ...
    if(!(name[length-1] == 'z' && name[length-2] == 'c'  && name[length-3] == 'h')){
        printf("Incorrect File.\n");
        return -1;
    }

    int fd2 = open(name, O_RDONLY, 0744);
    if(fd2 < 0){
        printf("File doesn't exist.\n");
            return -1;
    }
    ...
    char buffer1[100];
    char* buffer4;
    string_create(&buffer4,name,0,strlen(name)-4);
    printf("buffer is %s\n", buffer4);
    int fd3 = open(buffer4, O_WRONLY | O_CREAT, 0744);

    do {
        status = read(fd2, &b, 1);
        if(status == 0){ // reached the end of the file
            break;
        }
        buffer1[j] = b;
        j++;
        char* name1 = compareStringtoLinkedList(head, buffer1);
        
        if(name1){

            if(string_equal("$s", name1)){

                strcpy(name1, " ");
            }
            if(string_equal("$n", name1)){
                strcpy(name1, "\n");
            }
            if(string_equal("$t", name1)){
                strcpy(name1, "\t");
            }
        
            write(fd3,name1, strlen(name1));
            j = 0;
...
int recursiveDirectories2(char* name, char flag){
    ...
    while(currentElement != NULL){
        ...
        if(currentElement->d_type == 4){
            ...
            strcat(path, currentElement->d_name);
            //printf("path is %s\n", path);
            recursiveDirectories2(path, flag);
        }
        currentElement = readdir(directory);
    }
    free(path);
...
int main(int argc, char **argv)
{
    ...
    // ------------------------------------------------------------------------------------------------------
        ...
        if(flag == 'b'){
            ...
            if(ifrecursive == 1){
                recursiveDirectories2(argv[3], flag);
